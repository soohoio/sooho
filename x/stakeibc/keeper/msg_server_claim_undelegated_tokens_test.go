...
func (s *KeeperTestSuite) TestClaimUndelegatedTokens_NoUserRedemptionRecord() {
	...
	_, err := s.GetMsgServer().ClaimUndelegatedTokens(sdk.WrapSDKContext(s.Ctx), &tc.validMsg)
	s.Require().EqualError(err, "unable to find claimable redemption record for msg: creator:\"stayking_SENDER\" host_zone_id:\"GAIA\" epoch:1 sender:\"stayking_SENDER\" , error User redemption record GAIA.1.stayking_SENDER not found on host zone GAIA: user redemption record error: record not found")
}
...
func (s *KeeperTestSuite) TestClaimUndelegatedTokens_RecordNotClaimable() {
	...
	_, err := s.GetMsgServer().ClaimUndelegatedTokens(sdk.WrapSDKContext(s.Ctx), &tc.validMsg)
	s.Require().EqualError(err, "unable to find claimable redemption record for msg: creator:\"stayking_SENDER\" host_zone_id:\"GAIA\" epoch:1 sender:\"stayking_SENDER\" , error User redemption record GAIA.1.stayking_SENDER is not claimable, pending ack: user redemption record error: record not found")
}
...
func (s *KeeperTestSuite) TestClaimUndelegatedTokens_RecordNotFound() {
	...
	s.Require().EqualError(err, "unable to find claimable redemption record for msg: creator:\"stayking_SENDER\" host_zone_id:\"fake_host_zone\" epoch:1 sender:\"stayking_SENDER\" , error User redemption record fake_host_zone.1.stayking_SENDER not found on host zone fake_host_zone: user redemption record error: record not found")
}
...
func (s *KeeperTestSuite) TestClaimUndelegatedTokens_NoEpochTracker() {
	...
	expectedErr += "Epoch tracker not found for epoch stayking_epoch: epoch not found"
	fmt.Println()
...
func (s *KeeperTestSuite) TestClaimUndelegatedTokens_HzuNotStatusTransferred() {
	...
	_, err := s.GetMsgServer().ClaimUndelegatedTokens(sdk.WrapSDKContext(s.Ctx), &tc.validMsg)
	s.Require().EqualError(err, "unable to find claimable redemption record for msg: creator:\"stayking_SENDER\" host_zone_id:\"GAIA\" epoch:1 sender:\"stayking_SENDER\" , error User redemption record GAIA.1.stayking_SENDER is not claimable, host zone unbonding has status: EXIT_TRANSFER_QUEUE, requires status CLAIMABLE: user redemption record error: record not found")
}
