...
func (s *KeeperTestSuite) checkEpochElapsedShare(epochDurationSeconds float64, nextStartTimeSeconds float64, expectedShare sdk.Dec) {
	...
	actualShare, err := s.App.StakeibcKeeper.GetStayKingEpochElapsedShare(s.Ctx)
...
func (s *KeeperTestSuite) TestEpochElapsedShare_Failed_EpochNotFound() {
	...
	_, err := s.App.StakeibcKeeper.GetStayKingEpochElapsedShare(s.Ctx)
	s.Require().EqualError(err, "Failed to get epoch tracker for stayking_epoch: not found")
}
