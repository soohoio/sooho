...
func (k Keeper) GetPositionByStatusAndDepositRecordId(ctx sdk.Context, status types.PositionStatus, depositRecordId uint64) (position types.Position, found bool) {
	store := prefix.NewStore(ctx.KVStore(k.storeKey), types.KeyPrefixPosition)
	iterator := sdk.KVStorePrefixIterator(store, []byte{})
	found = false
	defer iterator.Close()
	...
	for ; iterator.Valid(); iterator.Next() {
		var val types.Position
		k.cdc.MustUnmarshal(iterator.Value(), &val)
		if val.GetStatus() == status && val.GetDepositRecordId() == depositRecordId {
			position = val
			found = true
			break
		}
...
func (k Keeper) Liquidate(ctx sdk.Context, loanId uint64) error {
	...
	if position.Status == types.PositionStatus_POSITION_PENDING {
		return errorsmod.Wrapf(types.ErrPositionIsNotActive, "position %v is pending status, so it can not be liquidated", position.Id)
	}

	// liquidation performance fee 와 fee 를 제외한 totalStAsset 을 position 에 다시 저장한다.
