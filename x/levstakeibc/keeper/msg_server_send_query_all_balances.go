...
func (k msgServer) SendQueryAllBalances(goCtx context.Context, msg *types.MsgSendQueryAllBalances) (*types.MsgSendQueryAllBalancesResponse, error) {
	...
	seq, err := k.InterchainQueryKeeper.SendQuery(ctx, "icacontroller-levstakeibc.osmosis-localnet.ICQ", msg.ChannelId, chanCap, reqs, clienttypes.ZeroHeight(), uint64(timeoutTimestamp))
	if err != nil {
		return nil, err
	}
