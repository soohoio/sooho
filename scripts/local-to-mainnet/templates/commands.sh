...
HOST_BINARY tx ibc-transfer transfer transfer $transfer_channel 수호계정 4000000HOST_DENOM --from hot --chain-id HOST_CHAIN_ID -y --keyring-backend test --node http://HOST_ENDPOINT:26657

# Confirm funds were recieved on stayking and get IBC denom
build/staykingd --home STAYKING_HOME q bank balances 수호계정

# Register host zone
IBC_DENOM=$(build/staykingd --home STAYKING_HOME q bank balances 수호계정 | grep ibc | awk '{print $2}' | tr -d '"') && echo $IBC_DENOM
...
balance=$(build/staykingd --home STAYKING_HOME q bank balances $fee_address | grep amount | awk '{print $3}' | tr -d '"') && echo $balance
