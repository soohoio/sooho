...
type AppTestHelper struct {
	...
	StayKingChain *ibctesting.TestChain
...
func (s *AppTestHelper) Setup() {
	...
	s.Ctx = s.App.BaseApp.NewContext(false, tmtypes.Header{Height: 1, ChainID: StayKingChainID})
...
func (s *AppTestHelper) SetupIBCChains(hostChainID string) {
	...
	s.StayKingChain = ibctesting.NewTestChain(s.T(), s.Coordinator, StayKingChainID)
	...
	s.Coordinator.Chains = map[string]*ibctesting.TestChain{
		StayKingChainID: s.StayKingChain,
...
func (s *AppTestHelper) CreateTransferChannel(hostChainID string) {
	...
	s.TransferPath = NewTransferPath(s.StayKingChain, s.HostChain)
	s.Coordinator.Setup(s.TransferPath)

	// Replace stayking and host apps with those from TestingApp
	s.App = s.StayKingChain.App.(*app.StayKingApp)
	s.HostApp = s.HostChain.GetSimApp()
	s.Ctx = s.StayKingChain.GetContext()
...
func (s *AppTestHelper) CreateICAChannel(owner string) string {
	...
	icaPath := NewIcaPath(s.StayKingChain, s.HostChain)
