...
set_stayking_genesis() {
    ...
    jq '(.app_state.epochs.epochs[] | select(.identifier=="stayking_epoch") ).duration = $epochLen' --arg epochLen $STAYKING_EPOCH_EPOCH_DURATION $genesis_config > json.tmp && mv json.tmp $genesis_config
...
set_host_genesis() {
    ...
    jq '.app_state.interchainquery.params.allow_queries += ["/cosmos.bank.v1beta1.Query/Balance","/osmosis.poolmanager.v1beta1.Query/EstimateSinglePoolSwapExactAmountOut","/osmosis.poolmanager.v1beta1.Query/EstimateSwapExactAmountOut","/osmosis.poolmanager.v1beta1.Query/EstimateSwapExactAmountIn","/osmosis.poolmanager.v1beta1.Query/EstimateSinglePoolSwapExactAmountIn"]' $genesis_config > json.tmp && mv json.tmp $genesis_config
