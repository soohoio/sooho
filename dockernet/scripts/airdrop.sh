...
# distributor address: sooho1z835j3j65nqr6ng257q0xkkc9gta72gf48txwl
...
echo "barrel salmon half click confirm crunch sense defy salute process cart fiscal sport clump weasel render private manage picture spell wreck hill frozen before" | \
    $STAYKING_MAIN_CMD keys add distributor-test --recover

# airdrop-test address: sooho1nf6v2paty9m22l3ecm7dpakq2c92ueyununayr
...
echo "royal auction state december october hip monster hotel south help bulk supreme history give deliver pigeon license gold carpet rabbit raw wool fatigue donate" | \
    $STAYKING_MAIN_CMD keys add airdrop-test --recover

## AIRDROP SETUP
echo "Funding accounts..."
# Transfer uatom from gaia to stayking, so that we can liquid stake later
$GAIA_MAIN_CMD tx ibc-transfer transfer transfer channel-0 sooho1nf6v2paty9m22l3ecm7dpakq2c92ueyununayr 1000000uatom --from ${GAIA_VAL_PREFIX}1 -y
sleep 5
# Fund the distributor account
$STAYKING_MAIN_CMD tx bank send val1 sooho1z835j3j65nqr6ng257q0xkkc9gta72gf48txwl 600000ustrd --from val1 -y
sleep 5
# Fund the airdrop account
$STAYKING_MAIN_CMD tx bank send val1 sooho1nf6v2paty9m22l3ecm7dpakq2c92ueyununayr 1000000000ustrd --from val1 -y
sleep 5
# Create the airdrop, so that the airdrop account can claim tokens
$STAYKING_MAIN_CMD tx claim create-airdrop stayking 1666792900 40000000 ustay --from distributor-test -y
sleep 5
# Set airdrop allocations
$STAYKING_MAIN_CMD tx claim set-airdrop-allocations stayking sooho1nf6v2paty9m22l3ecm7dpakq2c92ueyununayr 1 --from distributor-test -y
sleep 5

# AIRDROP CLAIMS
# Check balances before claims
echo "Initial balance before claim:"
$STAYKING_MAIN_CMD query bank balances sooho1nf6v2paty9m22l3ecm7dpakq2c92ueyununayr
# NOTE: You can claim here using the CLI, or from the frontend!
# Claim 20% of the free tokens
echo "Claiming fee amount..."
$STAYKING_MAIN_CMD tx claim claim-free-amount --from airdrop-test --gas 400000
sleep 5
echo "Balance after claim:" 
$STAYKING_MAIN_CMD query bank balances sooho1nf6v2paty9m22l3ecm7dpakq2c92ueyununayr
# Stake, to claim another 20%
echo "Staking..."
$STAYKING_MAIN_CMD tx staking delegate soohovaloper1nnurja9zt97huqvsfuartetyjx63tc5zrj5x9f 100ustrd --from airdrop-test --gas 400000
sleep 5
echo "Balance after stake:" 
$STAYKING_MAIN_CMD query bank balances sooho1nf6v2paty9m22l3ecm7dpakq2c92ueyununayr
# Liquid stake, to claim the final 60% of tokens
echo "Liquid staking..."
$STAYKING_MAIN_CMD tx stakeibc liquid-stake 1000 uatom --from airdrop-test --gas 400000
sleep 5
